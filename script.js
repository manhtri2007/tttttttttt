...
const state = {
  ...
  roundStats: {
    byColor: { grey: 0, green: 0, blue: 0, purple: 0, red: 0 },
    killTimes: [],
    graph: [],
    misses: 0
  },
...
function buildNonNegativeFormula() {
  ...
  if (!enabled.length) return { answer: 0, label: 'Bật ít nhất một chế độ', color: 'grey' };
  ...
  return { answer: clamp(Math.round(formula.answer), 0, 9999999), label: formula.label, color: mode.color };
}

function spawnDroplet() {
  if (state.droplets.length >= MAX_DROPLETS) return;

  const { answer, label, color } = buildNonNegativeFormula();
...
function renderHistory() {
  ...
  const totals = entries.reduce((acc, entry) => {
    ...
    return acc;
  }, { point: 0, destroyed: 0, duration: 0, games: 0, byColor: { grey: 0, green: 0, blue: 0, purple: 0, red: 0 } });
