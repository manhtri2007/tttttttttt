...
function buildNonNegativeFormula() {
  ...
  return { answer: clamp(Math.round(formula.answer), 0, 9999999), label: formula.label, color, isSpecial: color === 'gold' || color === 'pink' };
}
...
function activateSpecialDrop(drop) {
  ...
  const index = state.droplets.findIndex((item) => item.id === drop.id);
  if (index >= 0) {
    state.droplets[index].element.remove();
    state.droplets.splice(index, 1);
  }
  state.roundStats.byColor.pink = (state.roundStats.byColor.pink || 0) + 1;
  state.roundDestroyed += 1;
  state.score += scoreMap.pink;
  state.lives += 1;
  addExp(scoreMap.pink);
  clearInput();
  updateHUD();
}
