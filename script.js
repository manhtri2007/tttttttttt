...
function makeSubtraction(maxValue = 20, terms = 2) {
  const values = Array.from({ length: terms }, () => randomInt(0, maxValue));
  values.sort((a, b) => b - a);
  return { label: values.join(' - '), answer: Math.max(0, values.reduce((result, value) => result - value, values[0])) };
}
...
function makeMissing(operation, maxValue) {
  const answer = randomInt(0, maxValue);
  ...
  if (operation === '÷') return { label: `${other * answer} ÷ x = ${other}`, answer: answer || 1 };
...
function makeIntegerFormula(mode) {
  ...
  if (mode.index === 0) return { label: `${Math.abs(a)} - (${a})`, answer: Math.abs(a) - a };
  if (mode.index === 7) return { label: `${a} × ${a}`, answer: a * a };
  if (mode.index === 8) {
    const divisor = randomInt(1, range);
    const quotient = randomInt(0, range);
    return { label: `${divisor * quotient} ÷ ${divisor}`, answer: quotient };
  }
  if (mode.index === 5) return { label: `${a} + ${b} + ${randomInt(-range, range)}`, answer: Math.max(0, a + b) };
  return mode.index === 6 ? makeAddition(randomInt(3, 5), range) : (mode.index === 3 ? { label: `${a} - (${b})`, answer: Math.max(0, a - b) } : { label: `${a} + (${b})`, answer: Math.max(0, a + b) });
}
...
function buildNonNegativeFormula() {
  ...
  if (mode.group === 'addition') {
    ...
    else { const base = randomInt(-20, 20); const exponent = randomInt(0, 3); formula = { label: `${base}^${exponent}`, answer: Math.max(0, Math.pow(base, exponent)) }; }
  }

  const color = Math.random() < 0.18 ? randomChoice(['gold', 'pink']) : mode.color;
  return { answer: clamp(Math.round(Math.max(0, formula.answer)), 0, 9999999), label: formula.label, color, isSpecial: false };
}
